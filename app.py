...
# 그래프 생성 함수
def create_graph(category, data, normalized_data):
    ...
    fig.update_layout(
        title=f"{category} 추세 (2020-현재)",
        xaxis_title="날짜",
        legend=dict(x=1.05, y=1, xanchor="left", yanchor="top"),
        height=600,
        width=1000,
    )
    fig.update_yaxes(title_text="원본 값", secondary_y=False)
    fig.update_yaxes(title_text="정규화 값 (시작점 = 100)", secondary_y=True)
...
for category in categories:
    fig = create_graph(category, data, normalized_data)
    st.plotly_chart(fig)
...
fig.update_layout(
    title="모든 경제 지표의 정규화된 추세 (2020-현재)",
    xaxis_title="날짜",
    yaxis_title="정규화 값 (시작점 = 100)",
    legend=dict(x=1.05, y=1, xanchor="left", yanchor="top"),
    height=600,
    width=1000,
)
...
legend_explanation = """
범례 설명:
- TLT: 30년 만기 미국 국채 수익률
- DXY: 미국 달러 지수
- SPY: S&P 500 지수 ETF
- QQQ: 나스닥 100 지수 ETF
- VIX: 변동성 지수
- Fed_Reserve_Balances: 연방준비금 잔액
- Fed_Total_Assets: 연방준비제도 총자산
- Treasury_Cash: 재무부 현금 보유량
- Reverse_Repo: 역레포 거래량
"""

st.write(legend_explanation)
